...
public class Translacion03 extends JFrame implements KeyListener{
	...
	private int[] punto = {250,250,-300};
	private Cubo cubo;
	
	public Translacion03() {
	...
	public static void main(String[] args) {
		Translacion03 ventana = new Translacion03();
		ventana.Cubo();
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_LEFT:
			if(centro.getX()>0)dx=-4;
			break;
		case KeyEvent.VK_RIGHT:
			if(centro.getX()<getWidth())dx=4;
			break;
		case KeyEvent.VK_UP:
			if(centro.getY()>30)dy=-4;
			break;
		case KeyEvent.VK_DOWN:
			if(centro.getY()<getHeight())dy=4;
			break;
		case KeyEvent.VK_M:
			dz=-2;
			break;
		...
		if(cubo.getX()<0 && dx<0) dx = 0;
		if(cubo.getX()>getWidth() && dx>0) dx = 0;

		if(cubo.getY()<0 && dy<0) dy = 0;
		if(cubo.getY()>getHeight() && dy>0) dy = 0;

		cubo.transladar(dx, dy, dz);
		cubo.dibujar(Cubo.PERSPECTIVA, gPantalla, punto);
		finalizar();
	}
